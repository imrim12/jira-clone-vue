<template>
  <ul class="my-avatar-group" :class="[size ? `my-avatar-group--${size}` : '']">
    <li
      ...
      :class="[!value ? null : value.index === index ? 'active' : null]"
      @click="triggerAvatar({ avatar, index })"
    >
...
export default defineComponent({
  ...
  methods: {
    triggerAvatar({ avatar, index }) {
      if (this.value?.index === index) {
        this.$emit('change', null)
      } else {
        this.$emit('change', { ...avatar, index })
      }
    },
  },
})
